...
// 文件内宏定义
// 高斯平滑核
#define SEGMENT_USE_5X5KERNAL
// 条码方向梯度特征点累计阈值，可调
#define PARALLEL_CAST_THRESHOLD_BASIC	(38)
#define PARALLEL_CAST_THRESHOLD_RATIO	(0.6)
#define BARCODE_ONTERCEPT_EXTEND		(5)		// 半个最小空白区宽度
#define BARCODE_INTERCEPT_EXTEND		(3)		// 半个最小空白区宽度

#define SEGMENT_ZOOM_WIDTH_THRESH		(300)
#define SEGMENT_ZOOM_RATIO_THRESH		(0.6F)

#define SEGMENT_EXTEND_WIDTH_RATIO		(0.1F)

#define SEGMENT_HOUGH_ANGLE_RADIUS		(18)	// 霍夫直线检测角度半径
#define nSgmNumangle 	 				(37)	// SEGMENT_HOUGH_ANGLE_RADIUS * 2 + 1

#define nSgmInterceptMaxCount			(8)
#define nSgmOnterceptMaxCount			(32)


static int nSgmImgMaxWidth = 0, nSgmImgMaxHeight = 0;

static unsigned char * ucSgmParallelMask = 0, * ucSgmVerticalMask = 0;

static RyuPoint * pptSgmImgEffcRowRanges = 0;

static int nSgmHoughSeqMaxSize = 0;
static RyuPoint * pptSgmVerticalSeq = 0;
static int nSgmVerticalSeqCnt = 0;
static RyuPoint * pptSgmParallelSeq = 0;
static int nSgmParallelSeqCnt = 0;

static int * nSgmHoughAccum = 0;

static int * pnSgmCastAccumVerti = 0;
static int * pnSgmCastAccumParal = 0;

static int * pnSgmMedianArray1 = 0, * pnSgmMedianArray2 = 0;

SegmentBarcodeArea * stSegmentBarcodeArea = 0;
...
int GetBarcodePureAreaFigures( RyuPoint * intercept, int * param1, int count1,
	...
	RyuPoint main_intrcpt, RyuPoint main_ontrcpt, int barcode_area_idx);
		...
		int barcode_area_idx)
{
	...
	RyuPoint barcodeIntercept[nSgmInterceptMaxCount];
	RyuPoint barcodeOntercept[nSgmInterceptMaxCount];
	int intercept_cnt = 0, ontercept_cnt = 0;
	int reference = 0;
	int param1[nSgmInterceptMaxCount] = {0};
	int param2[nSgmInterceptMaxCount] = {0};
	...
	status = FindRectangleEffectiveRange( corner, LTRB, pptSgmImgEffcRowRanges );
	if( status <= 0 ) {
		nRet = 0;
		goto nExit;
	}
	...
	status = RectRangeAdaptImageFigure( LTRB, pptSgmImgEffcRowRanges, width, height );
	if( status <= 0 ) {
		nRet = 0;
		goto nExit;
	}
	...
	status = GetSpecifiedGradientMasks( in_data, width, height, LTRB, pptSgmImgEffcRowRanges,
				apprx_angle, ref_grad, ucSgmParallelMask, ucSgmVerticalMask );
	if( status <= 0 ) {
		nRet = 0;
		goto nExit;
	}
	...
	status = CorrectAngleViaStandardHoughTrans( pptSgmVerticalSeq, nSgmVerticalSeqCnt, LTRB, apprx_angle, &exact_angle );
	if(status < 0) {
		nRet = 0;
		goto nExit;
	}
	...
	status = CastPointsAlongSpecifiedOrientation( pptSgmParallelSeq, nSgmParallelSeqCnt, LTRB, exact_angle,
				pnSgmCastAccumParal, pnSgmCastAccumVerti );
	if(status <= 0) {
		nRet = 0;
		goto nExit;
	}
	...
	// 定位条码上下界
	status = PressUponBarcodeIntercept( pnSgmCastAccumParal, LTRB, barcodeIntercept, param1 );
	if(status <= 0) {
		nRet = 0;
		goto nExit;
	}
	...
	// 定位条码左右界
	status = PressUponBarcodeOntercept( pnSgmCastAccumVerti, LTRB, reference, barcodeOntercept, param2 );
	if(status <= 0) {
		nRet = 0;
		goto nExit;
	}
	...
	status = GetBarcodePureAreaFigures( barcodeIntercept, param1, intercept_cnt,
		barcodeOntercept, param2, ontercept_cnt, LTRB, exact_angle, *intercept, *ontercept,
		barcode_area_idx);

	*angle = exact_angle;

	if(status > 0)
		Cache_wb(stSegmentBarcodeArea+barcode_area_idx, status * sizeof(SegmentBarcodeArea), Cache_Type_ALL, TRUE);
...
{
	nSgmImgMaxWidth 		= globalPtr->nBarcodeDetectInitImgWid;
	nSgmImgMaxHeight 		= globalPtr->nBarcodeDetectInitImgHei;
	nSgmHoughSeqMaxSize 	= globalPtr->nSgmHoughSeqMaxSize;
	pptSgmImgEffcRowRanges 	= (RyuPoint *)globalPtr->pptSgmImgEffcRowRanges;
	pptSgmVerticalSeq 		= (RyuPoint *)globalPtr->pptSgmVerticalSeq;
	pptSgmParallelSeq 		= (RyuPoint *)globalPtr->pptSgmParallelSeq;
	nSgmHoughAccum 			= globalPtr->nSgmHoughAccum;
	pnSgmCastAccumVerti 	= globalPtr->pnSgmCastAccumVerti;
	pnSgmCastAccumParal 	= globalPtr->pnSgmCastAccumParal;
	pnSgmMedianArray1 		= globalPtr->pnSgmMedianArray1;
	pnSgmMedianArray2 		= globalPtr->pnSgmMedianArray2;
	stSegmentBarcodeArea 	= (SegmentBarcodeArea *)globalPtr->stSegmentBarcodeArea;
							  ...
							  int apprx_angle, int ref_grad, unsigned char * parallel_mask, unsigned char * vertical_mask )
{
	...
	const int anglthres = SEGMENT_HOUGH_ANGLE_RADIUS;

	unsigned char * pData = 0;
	//unsigned char * pParal = 0, * pVerti = 0;

	int base = 0, dx = 0, dy = 0;
	//int theta = 0;

	RyuPoint * verticalSeq = pptSgmVerticalSeq, * parallelSeq = pptSgmParallelSeq;
	...
	offset1[0] = width; offset1[1] = width-1; offset1[2] = width-1;  offset1[3] = -1;
	offset1[4] = 1;  	offset1[5] = 1;		  offset1[6] = width+1;	 offset1[7] = width; offset1[8] = width;

	offset2[0] = -width; offset2[1] = -width; 	offset2[2] = -width+1;  offset2[3] = -width+1;
	offset2[4] = -1;  	offset2[5] = -width-1;	offset2[6] = -width-1;	offset2[7] = -width-1; offset2[8] = -width;

	offset1_t[0] = 1; 		offset1_t[1] = 1; 		offset1_t[2] = width+1;  offset1_t[3] = width;
	offset1_t[4] = width;  	offset1_t[5] = width-1;	offset1_t[6] = width-1;	 offset1_t[7] = -1;		offset1_t[8] = 1;

	offset2_t[0] = -1; 		offset2_t[1] = -width-1; offset2_t[2] = -width-1;  offset2_t[3] = -width-1;
	offset2_t[4] = -width;  offset2_t[5] = -width;	 offset2_t[6] = -width+1;  offset2_t[7] = -width+1; offset2_t[8] = -1;
	...
	if( 0 >= tmp1 && 0 < tmp2) {
		range1.x = range2.x = -ryuAtanLUTHalf(-tmp1);
		range1.y = range2.y = ryuAtanLUTHalf(tmp2);
	} else if( 0 < tmp1 && 90 > tmp2 ) {
		range1.x = range2.x = ryuAtanLUTHalf(tmp1-1);
		range1.y = range2.y = ryuAtanLUTHalf(tmp2);
	} else if( 90 >= tmp1 && 90 <= tmp2 ) {
		range1.x = ryuAtanLUTHalf(tmp1-1);
		range1.y = 256 << TRIGONOMETRIC_SHIFT_DIGIT;
		range2.x = -range1.y;
		range2.y = -ryuAtanLUTHalf(179-tmp2);
	} else if( 90 < tmp1 && 180 > tmp2 ) {
		range1.x = range2.x = -ryuAtanLUTHalf(179-tmp1);
		range1.y = range2.y = -ryuAtanLUTHalf(179-tmp2);
	} else if( 180 > tmp1 && 180 <= tmp2) {
		range1.x = range2.x = -ryuAtanLUTHalf(179-tmp1);
		range1.y = range2.y = ryuAtanLUTHalf(tmp2-180);
	} else {
		nRet = -1;
		goto nExit;
	}
	...
	nColCount = LTRB[3] - LTRB[1] + 1;
	nSgmVerticalSeqCnt = nSgmParallelSeqCnt = 0;
...
// 			if( anglthres >= ryuCycleDistance( theta, apprx_angle, 180 )
			...
			if( ((tmp1 >= range1.x && tmp1 <= range1.y) || (tmp1 >= range2.x && tmp1 <= range2.y))
				...
				&& nSgmParallelSeqCnt < nSgmHoughSeqMaxSize) {
//				*pParal = 0xff;
				parallelSeq[nSgmParallelSeqCnt].x = i;
				parallelSeq[nSgmParallelSeqCnt].y = j;
				nSgmParallelSeqCnt++;
			}
//			else {
//				*pParal = 0;
//			}
			...
			if( gradthres <= abs(dx) && gradthres > abs(dy)
				&& nSgmVerticalSeqCnt < nSgmHoughSeqMaxSize ) {
//				*pVerti = 0xff;
				verticalSeq[nSgmVerticalSeqCnt].x = i;
				verticalSeq[nSgmVerticalSeqCnt].y = j;
				nSgmVerticalSeqCnt++;
			}
//			else {
//				*pVerti = 0;
...
//			pVerti++;
	...
	printf("Find %d effective Parallel points, and %d effective Vertical points\n",
		nSgmParallelSeqCnt, nSgmVerticalSeqCnt);
		...
		int apprx_angle, int * exact_angle )
{
	...
	int * adata0 = nSgmHoughAccum, *adata = 0;

	const int fixedhalf = 1 << (TRIGONOMETRIC_SHIFT_DIGIT - 1);
	const float lineThreshRt = 0.5;

	int angle[nSgmNumangle] = {0};
	int sine[nSgmNumangle] = {0};
	int cosine[nSgmNumangle] = {0};

	int linecnt[nSgmNumangle] = {0};
	int elemcnt[nSgmNumangle] = {0};

	int maxLinecnt = 0, maxLinecntIdx = 0, maxElemcnt = 0, maxElemcntIdx = 0;

	memset( adata0, 0, nSgmNumangle * numrho * sizeof(int) );

	for( n = 0; n < nSgmNumangle; n++ ) {
		angle[n] = ryuCycle( apprx_angle-SEGMENT_HOUGH_ANGLE_RADIUS+n, 180 );
	...
	for( i = 0; i < seq_count; i++ ) {
		adata = adata0;
		// 对此点逐行更新累加器
		for( n = 0; n < nSgmNumangle; n++, adata += numrho ) {
		...
		int correct_angle )
{
	...
	//////////////////////////////////////////////////////////////////////////
	// 注释部分
//	C[0]  = A *  corner[0].x + B *  corner[0].y + fixedhalf;
//	CT[0] = AT * corner[0].x + BT * corner[0].y + fixedhalf;
//
...
int PressUponBarcodeIntercept( int * cast_array, int * LTRB, RyuPoint * intercept, int * param)
{
	...
	int max_count = nSgmInterceptMaxCount;	// 最多支持8个，再多则抛弃
	RyuPoint ptStartstop[nSgmInterceptMaxCount];
	int avg_vals[nSgmInterceptMaxCount] = {0};
	RyuPoint ptTemp;
...
int PressUponBarcodeOntercept( int * cast_array, int * LTRB, int reference, RyuPoint * ontercept, int * param )
{
	...
	int * medianArrayVal = pnSgmMedianArray1;
	int * medianArrayDist = pnSgmMedianArray2;
	...
	const int maxCountGlobal = nSgmInterceptMaxCount;
	//const int nMaxCount = nSgmOnterceptMaxCount;	// 最多支持32个，再多则抛弃
	RyuPoint ptStartstop[nSgmOnterceptMaxCount];
	int max_vals[nSgmOnterceptMaxCount] = {0};
	int pnEffCnt[nSgmOnterceptMaxCount] = {0};
	int count = 0;
	RyuPoint ptTemp;
		...
		RyuPoint main_intrcpt, RyuPoint main_ontrcpt, int barcode_area_idx)
{
	...
	SegmentBarcodeArea * barcode_area = stSegmentBarcodeArea + barcode_area_idx;
...
// 			}
			...
			tempSegment.ex_symbol = tempSegment.ex_startbit = tempSegment.ex_checkbit = 0;

			barcode_area[totalCnt++] = tempSegment;
#ifdef	_DEBUG_
#ifdef  _DEBUG_SEGMENT
			printf("条码区域%d: ratio=%.3f, dist=%d, flag=%d\n", totalCnt, ratio, dist, tempSegment.zoom);
...
#endif
#endif
			if(totalCnt + barcode_area_idx >= MAX_SEGMENT_SLAVE_COUNT)
				break;
		}
		if(totalCnt + barcode_area_idx >= MAX_SEGMENT_SLAVE_COUNT)
			break;
