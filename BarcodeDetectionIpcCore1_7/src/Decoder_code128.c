...
#define CODE128_MAX_MODULE_DEVIATION	(2<<FLOAT2FIXED_SHIFT_DIGIT)
#define CODE128_MAXMIN_MODULE_RATIO		(1.35)
// END

const int gnDecoderModuleCode128[108] = {
...
int GetOcrParams_code128(int * decode_arr, int arr_count, int * startbit, int * checkbit);
		...
		int * ex_flag, int * startbit, int * checkbit)
{
	...
	if(!decode_arr || !code_result || !code_digit || !code_module
		...
		|| !ex_flag || !startbit || !checkbit) {
			...
			printf( "ERROR! Invalid input of RecgCode128, decode_arr=0x%x, code_result=0x%x\
					...
					, code_idxR=0x%x, ex_flag=0x%x, startbit=0x%x, checkbit=0x%x\n",
					decode_arr, code_result, code_digit, code_module,
					code_direct, code_idxL, code_idxR, ex_flag, startbit, checkbit);
#endif
			nRet = -1;
			goto nExit;
	}
	...
	*ex_flag = *startbit = *checkbit = 0;
	...
	nExflag = GetOcrParams_code128(decode_arr+nStartIdx, nArrCount, &nStartbit, &nCheckbit);

	if( nArrCount < CODE128_MIN_DECODEARR_COUNT) {
#ifdef	_DEBUG_
#ifdef  _DEBUG_DECODER_CODE128
		printf("Cannot find code128, too small nColCount: %d\n", nArrCount);
#endif
#endif
		nRet = 0;
		goto nExit;
	}
	else if(0 != (nArrCount-1) % 6) {
#ifdef	_DEBUG_
#ifdef  _DEBUG_DECODER_CODE128
...
int CodeStartStopMatch_code128( int * decode_arr, int arr_count,  int * start_idx, int * stop_idx )
{
	...
	*start_idx = *stop_idx = 0;
	...
	for(i = 0; i < RYUMIN(arr_count - 6, 10); i++) {
...
int GetOcrParams_code128(int * decode_arr, int arr_count, int * startbit, int * checkbit)
{
	...
	*startbit = *checkbit = 0;
...
#ifdef	_DEBUG_
#ifdef  _DEBUG_DECODER_CODE128
		printf("Cannot match code128, unexpected value of pCodeCol[0] in GetOcrParams_code128, \
...
#ifdef	_DEBUG_
#ifdef  _DEBUG_DECODER_CODE128
		printf("Cannot match code128, unexpected value of s in GetOcrParams_code128, \
			   s=%d (should be >0)\n", s);
#endif
#endif
		return 0;
...
#ifdef	_DEBUG_
#ifdef  _DEBUG_DECODER_CODE128
		printf("Cannot match code128, unexpected value of nStartbit in GetOcrParams_code128, \
			   nStartbit=%d (should be in 103~105)\n", nStartbit);
#endif
#endif
		return 0;
	}
	...
	// 提取校验位
	pCodeCol = decode_arr + arr_count - 13;
...
#ifdef	_DEBUG_
#ifdef  _DEBUG_DECODER_CODE128
		printf("Cannot match code128, unexpected value of nCheckbit in GetOcrParams_code128, \
			   nCheckbit=%d (should be <106)\n", nCheckbit);
#endif
#endif
		return 0;
	}
