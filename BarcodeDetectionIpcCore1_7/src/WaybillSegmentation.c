...
int mapWaybillSegmentGlobalPtrs(BarcodeGlobalPointers * globalPtr)
{
	gnFloodFillMatMaxSize = globalPtr->nFloodFillMatMaxSize;
	gnFloodFillStatMaxCount = globalPtr->nFloodFillStatMaxCount;
	gucFloodFillImage = globalPtr->ucFloodFillImage;
	gptFloodFillSeeds = (RyuPoint *)globalPtr->ptFloodFillSeeds;
	gnFloodFillLabelMat = globalPtr->nFloodFillLabelMat;
	gtFloodFillStatistics = (FFN *)globalPtr->tFloodFillStatistics;
	gnFloodFillContourMat = globalPtr->nFloodFillContourMat;
	gnFloodFillContourFlag = globalPtr->nFloodFillContourFlag;
	gptFloodFillContourPts = (RyuPoint *)globalPtr->ptFloodFillContourPts;
	gnFloodFillClassSeeds = globalPtr->nFloodFillClassSeeds;
	gtFloodFillClassClusters = (FFC *)globalPtr->tFloodFillClassClusters;
	gtFloodFillCodeAreas = (CAN *)globalPtr->tFloodFillCodeAreas;
...
int WaybillSegment(unsigned char * img, int wid, int hei, RyuRect * bound_box)
{
	...
	if(wid >= hei) {
		procWid = WAYBILLSEG_RESIZE_SCALE;
		zoomRatio = procWid * 1.0 / wid;
		procHei = (int)(hei * zoomRatio);
	} else {
		procHei = WAYBILLSEG_RESIZE_SCALE;
		zoomRatio = procHei * 1.0 / hei;
		procWid = (int)(wid * zoomRatio);
	}

	imageProc = ryuCreateImageHeader(ryuSize(procWid, procHei), RYU_DEPTH_8C, 1);
	imageProc->imagedata = gucFloodFillImage;
...
//	}
	...
	if(FFStatisticsCount <= 0) {
		...
		bound_box->width = 1.0 * (FFStatistics[0].max_x - FFStatistics[0].min_x + 1) / zoomRatio;
		bound_box->height = 1.0 * (FFStatistics[0].max_y - FFStatistics[0].min_y + 1) / zoomRatio;
		nRet = 1;
		goto nExit;
	}
	...
	if(classCount > 1) {
		...
		bound_box->width = 1.0 * (FFClassClusters[1].max_x - FFClassClusters[1].min_x + 1) / zoomRatio;
		bound_box->height = 1.0 * (FFClassClusters[1].max_y - FFClassClusters[1].min_y + 1) / zoomRatio;
	}
...
nExit:
	...
	if(imageProc)
		ryuReleaseImageHeader(&imageProc);
	return nRet;
}
...
int WaybillSegm_pushCodeAreaStack(RyuPoint * corner, int angle, int flag, char * str_code)
{
	...
	for(i = 0; i < 4; i++) {
		if(corner[i].x > 0xf000)
			corner[i].x = corner[i].x | 0xffff0000;
		if(corner[i].y > 0xf000)
			corner[i].y = corner[i].y | 0xffff0000;
	}
...
int WaybillSegm_codeinSimilarity(FFN * ffn1, FFN * ffn2)
{
	...
	for(i = 0; i < ffn1->code_cnt; i++) {
		...
		for(j = 0; j < ffn2->code_cnt; j++) {
			...
			if(0 == status)
				return 1;
			status = ryuCycleDistance(FFCodeAreas[IndexArr1[i]].angle, FFCodeAreas[IndexArr2[j]].angle, 360);
			// 方向一致
			if(status <= 6)
				return 2;
		}
